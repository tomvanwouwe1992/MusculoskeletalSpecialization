...
class bounds:
    ...
    def getBoundsPosition(self):
        ...
        for count, joint in enumerate(self.joints):
            ...
            if joint == 'pelvis_tilt':
                ...
                upperBoundsPosition[joint] = 3.9 * np.pi / 180
                ...
                lowerBoundsPosition[joint] = -9.6 * np.pi / 180
                upperBoundsPosition[joint] = 14.9 * np.pi / 180
                ...
                lowerBoundsPosition[joint] = -29 * np.pi / 180
                upperBoundsPosition[joint] = 27 * np.pi / 180
                ...
                upperBoundsPosition[joint] = 31.53 * np.pi / 180
    ...
    def getBoundsVelocity(self):
        ...
        for count, joint in enumerate(self.joints):
            ...
            basicLimit = 1000
            if joint == 'pelvis_tilt':
                lowerBoundsVelocity[joint] = -100 * np.pi / 180
                upperBoundsVelocity[joint] = 100 * np.pi / 180
                scalingVelocity.insert(count, joint, [70.6 * np.pi / 180])
            elif joint == 'pelvis_list':
                lowerBoundsVelocity[joint] = -355.7 * np.pi / 180
                upperBoundsVelocity[joint] = 322 * np.pi / 180
                scalingVelocity.insert(count, joint, [355.7 * np.pi / 180])

            elif joint == 'pelvis_rotation':
                lowerBoundsVelocity[joint] = -171.7 * np.pi / 180
                upperBoundsVelocity[joint] = 171.5 * np.pi / 180
                scalingVelocity.insert(count, joint, [171.7 * np.pi / 180])

            elif joint == 'pelvis_tx':
                lowerBoundsVelocity[joint] = 0
                upperBoundsVelocity[joint] = 10
                scalingVelocity.insert(count, joint, [4])

            elif joint == 'pelvis_ty':
                lowerBoundsVelocity[joint] = -2
                upperBoundsVelocity[joint] = 2
                scalingVelocity.insert(count, joint, [1.375])

            elif joint == 'pelvis_tz':
                lowerBoundsVelocity[joint] = -1
                upperBoundsVelocity[joint] = 1
                scalingVelocity.insert(count, joint, [0.48])

            elif joint == 'lumbar_extension':
                lowerBoundsVelocity[joint] = -184.3 * np.pi / 180
                upperBoundsVelocity[joint] = 199 * np.pi / 180
                scalingVelocity.insert(count, joint, [199 * np.pi / 180])

            elif joint == 'lumbar_rotation':
                lowerBoundsVelocity[joint] = -154.2 * np.pi / 180
                upperBoundsVelocity[joint] = 206.3 * np.pi / 180
                scalingVelocity.insert(count, joint, [206.3 * np.pi / 180])

            elif joint == 'lumbar_bending':
                lowerBoundsVelocity[joint] = -402.9 * np.pi / 180
                upperBoundsVelocity[joint] = 442.5 * np.pi / 180
                scalingVelocity.insert(count, joint, [442.5 * np.pi / 180])

            elif (joint == 'hip_flexion_l') or (joint == 'hip_flexion_r'):
                lowerBoundsVelocity[joint] = -1180 * np.pi / 180
                upperBoundsVelocity[joint] = 1252 * np.pi / 180
                scalingVelocity.insert(count, joint, [1252 * np.pi / 180])

            elif (joint == 'hip_adduction_l') or (joint == 'hip_adduction_r'):
                lowerBoundsVelocity[joint] = -585.5 * np.pi / 180
                upperBoundsVelocity[joint] = 587 * np.pi / 180
                scalingVelocity.insert(count, joint, [587 * np.pi / 180])

            elif (joint == 'hip_rotation_l') or (joint == 'hip_rotation_r'):
                lowerBoundsVelocity[joint] = -382.1 * np.pi / 180
                upperBoundsVelocity[joint] = 399 * np.pi / 180
                scalingVelocity.insert(count, joint, [399 * np.pi / 180])

            elif (joint == 'knee_angle_l') or (joint == 'knee_angle_r'):
                lowerBoundsVelocity[joint] = -2227.9 * np.pi / 180
                upperBoundsVelocity[joint] = 2263 * np.pi / 180
                scalingVelocity.insert(count, joint, [2263 * np.pi / 180])

            elif (joint == 'ankle_angle_l') or (joint == 'ankle_angle_r'):
                lowerBoundsVelocity[joint] = -1489 * np.pi / 180
                upperBoundsVelocity[joint] = 1384 * np.pi / 180
                scalingVelocity.insert(count, joint, [1489 * np.pi / 180])

            elif (joint == 'subtalar_angle_l') or (joint == 'subtalar_angle_r'):
                lowerBoundsVelocity[joint] = -853.2 * np.pi / 180
                upperBoundsVelocity[joint] = 840.5 * np.pi / 180
                scalingVelocity.insert(count, joint, [853.2 * np.pi / 180])

            elif (joint == 'mtp_angle_l') or (joint == 'mtp_angle_r'):
                lowerBoundsVelocity[joint] = -744.9 * np.pi / 180
                upperBoundsVelocity[joint] = 744.9 * np.pi / 180
    ...
    def getBoundsAcceleration(self):
        ...
        for count, joint in enumerate(self.joints):
            ...
            basicLimit = 1000
            if joint == 'pelvis_tilt':
                lowerBoundsAcceleration[joint] = -5000 * np.pi / 180
                upperBoundsAcceleration[joint] = 5000 * np.pi / 180
                scalingAcceleration.insert(count, joint, [1133 * np.pi / 180])

            elif joint == 'pelvis_list':
                lowerBoundsAcceleration[joint] = -4948.5 * np.pi / 180
                upperBoundsAcceleration[joint] = 5273 * np.pi / 180
                scalingAcceleration.insert(count, joint, [5273 * np.pi / 180])

            elif joint == 'pelvis_rotation':
                lowerBoundsAcceleration[joint] = -3526.5 * np.pi / 180
                upperBoundsAcceleration[joint] = 2938 * np.pi / 180
                scalingAcceleration.insert(count, joint, [3526.5 * np.pi / 180])

            elif joint == 'pelvis_tx':
                lowerBoundsAcceleration[joint] = -61.54
                upperBoundsAcceleration[joint] = 51.3
                scalingAcceleration.insert(count, joint, [61.36])

            elif joint == 'pelvis_ty':
                lowerBoundsAcceleration[joint] = -30
                upperBoundsAcceleration[joint] = 30
                scalingAcceleration.insert(count, joint, [16.9])

            elif joint == 'pelvis_tz':
                lowerBoundsAcceleration[joint] = -5.91
                upperBoundsAcceleration[joint] = 5.91
                ...
                lowerBoundsAcceleration[joint] = -6288 * np.pi / 180
                upperBoundsAcceleration[joint] = 2852 * np.pi / 180
                scalingAcceleration.insert(count, joint, [6288 * np.pi / 180])

            elif (joint == 'hip_flexion_l') or (joint == 'hip_flexion_r'):
                lowerBoundsAcceleration[joint] = -20055 * np.pi / 180
                upperBoundsAcceleration[joint] = 20542 * np.pi / 180
                scalingAcceleration.insert(count, joint, [11542 * np.pi / 180])

            elif (joint == 'hip_adduction_l') or (joint == 'hip_adduction_r'):
                lowerBoundsAcceleration[joint] = -20092 * np.pi / 180
                upperBoundsAcceleration[joint] = 20339 * np.pi / 180
                scalingAcceleration.insert(count, joint, [8339 * np.pi / 180])

            elif (joint == 'hip_rotation_l') or (joint == 'hip_rotation_r'):
                lowerBoundsAcceleration[joint] = -20750 * np.pi / 180
                upperBoundsAcceleration[joint] = 20842 * np.pi / 180
                scalingAcceleration.insert(count, joint, [7842 * np.pi / 180])

            elif (joint == 'knee_angle_l') or (joint == 'knee_angle_r'):
                lowerBoundsAcceleration[joint] = -24944 * np.pi / 180
                upperBoundsAcceleration[joint] = 25312 * np.pi / 180
                scalingAcceleration.insert(count, joint, [25312 * np.pi / 180])

            elif (joint == 'ankle_angle_l') or (joint == 'ankle_angle_r'):
                lowerBoundsAcceleration[joint] = -50000 * np.pi / 180
                upperBoundsAcceleration[joint] = 50000 * np.pi / 180
                scalingAcceleration.insert(count, joint, [24657 * np.pi / 180])

            elif (joint == 'subtalar_angle_l') or (joint == 'subtalar_angle_r'):
                lowerBoundsAcceleration[joint] = -13247 * np.pi / 180
                upperBoundsAcceleration[joint] = 13344 * np.pi / 180
                scalingAcceleration.insert(count, joint, [13344 * np.pi / 180])

            elif (joint == 'mtp_angle_l') or (joint == 'mtp_angle_r'):
                lowerBoundsAcceleration[joint] = -28648 * np.pi / 180
                upperBoundsAcceleration[joint] = 28648 * np.pi / 180
                scalingAcceleration.insert(count, joint, [28648 * np.pi / 180])

            elif (joint == 'elbow_flex_l') or (joint == 'elbow_flex_r'):
                lowerBoundsAcceleration[joint] = -20000 * np.pi / 180
                upperBoundsAcceleration[joint] = 20000 * np.pi / 180
                scalingAcceleration.insert(count, joint, [5401 * np.pi / 180])

            elif (joint == 'arm_add_l') or (joint == 'arm_add_r'):
                lowerBoundsAcceleration[joint] = -5259.6 * np.pi / 180
                upperBoundsAcceleration[joint] = 5252 * np.pi / 180
                scalingAcceleration.insert(count, joint, [2259.6 * np.pi / 180])

            elif (joint == 'arm_rot_l') or (joint == 'arm_rot_r'):
                lowerBoundsAcceleration[joint] = -10582 * np.pi / 180
                upperBoundsAcceleration[joint] = 10533 * np.pi / 180
                scalingAcceleration.insert(count, joint, [6582 * np.pi / 180])

            elif (joint == 'arm_flex_l') or (joint == 'arm_flex_r'):
                lowerBoundsAcceleration[joint] = -10000 * np.pi / 180
                upperBoundsAcceleration[joint] = 10000 * np.pi / 180
