...
if NeMu_accurate:
    cases_sprinting = ['11','12','15']
    cases_marathon = ['13','16','14']
    cases_models = ['11','12','14']
    cases_sprinting_MUSC = ['11','18','20']
    cases_marathon_MUSC = ['13','19','17']
    cases_models_MUSC = ['11','18','17']
else:
    cases_sprinting = ['1','2','5']
    cases_marathon = ['3','6','4']
    cases_models = ['1','2','4']
    cases_sprinting_MUSC = ['1','8','10']
    cases_marathon_MUSC = ['3','9','7']
    cases_models_MUSC = ['1','8','7']
plot_SKEL = True
plot_MUSC = True
...
labels_models_MUSC = ('GEN', 'SPRINT_MUSC', 'MARATHON_MUSC')
...
if plot_SKEL:
    ...
    for i in range(len(cases_marathon)):
        y_value = np.around(optimaltrajectories[cases_sprinting_MUSC[i]]['Q_at_collocation'][3,-1], decimals=2).flatten()
    ...
    plt.savefig('figures_paper/MUSC_sprinting_step_length.svg',format = 'svg')
    plt.savefig('figures_paper/MUSC_sprinting_step_length.jpg',format = 'jpg', dpi = 600)
    ...
    for i in range(len(cases_marathon)):
        y_value = np.around(1 / optimaltrajectories[cases_sprinting_MUSC[i]]['time_col_opt'][-1], decimals=2).flatten()
        bar_i = ax.bar(x = x_values[i] , height = y_value, width = 0.45, color = colormap_MUSC[i], label = labels_models[i])
        ax.bar_label(bar_i) # add bar label
    ...
    # generate legend
    plt.legend(labels_models,loc='upper left',bbox_to_anchor=(1,0.5), fontsize = 10)
    plt.title('step frequency [Hz]')
    plt.savefig('figures_paper/MUSC_sprinting_step_frequency.svg',format = 'svg')
    plt.savefig('figures_paper/MUSC_sprinting_step_frequency.jpg',format = 'jpg', dpi = 600)
    ...
    for i in range(len(cases_marathon)):
        y_value = np.around(optimaltrajectories[cases_sprinting[i]]['Q_at_collocation'][3,-1], decimals=2).flatten()
        bar_i = ax.bar(x = x_values[i] , height = y_value, width = 0.45, color = colormap_MUSC[i], label = labels_models[i])
        ax.bar_label(bar_i) # add bar label
    ...
    plt.legend(labels_models,loc='upper left',bbox_to_anchor=(1,0.5), fontsize = 10)
    plt.title('step length [m]')
    plt.savefig('figures_paper/SKEL_sprinting_step_length.svg',format = 'svg')
    plt.savefig('figures_paper/SKEL_sprinting_step_length.jpg',format = 'jpg', dpi = 600)
    ...
    for i in range(len(cases_marathon)):
        y_value = np.around(1 / optimaltrajectories[cases_sprinting[i]]['time_col_opt'][-1], decimals=2).flatten()
        bar_i = ax.bar(x = x_values[i] , height = y_value, width = 0.45, color = colormap_MUSC[i], label = labels_models[i])
        ax.bar_label(bar_i) # add bar label
    ...
    plt.legend(labels_models,loc='upper left',bbox_to_anchor=(1,0.5), fontsize = 10)
    plt.title('step frequency [Hz]')
    plt.savefig('figures_paper/SKEL_sprinting_step_frequency.svg',format = 'svg')
    plt.savefig('figures_paper/SKEL_sprinting_step_frequency.jpg',format = 'jpg', dpi = 600)
    ...
    for i in range(len(sprinter_names)):
        ...
        if i == 1:
            ax.text(x_values_sprinter[i] - 0.03, sprinters_mass[i] + 0.7, sprinter_names[i], fontsize = 8)
        if i == 2:
            ax.text(x_values_sprinter[i] - 0.10, sprinters_mass[i] + 0.7, sprinter_names[i], fontsize = 8)
        if i == 3:
            ax.text(x_values_sprinter[i] - 0.03, sprinters_mass[i] + 0.7, sprinter_names[i], fontsize=8)
        if i == 4:
    ...
    for i in range(len(sprinter_names)):
        ...
        if i < 5:
            ax.text(x_values_sprinter[i] - 0.05, sprinters_lengths[i] + 0.005, sprinter_names[i], fontsize = 8)
        elif i < 6:
            ax.text(x_values_sprinter[i] - 0.1, sprinters_lengths[i] + 0.005, sprinter_names[i], fontsize = 8)
        else:
            ax.text(x_values_sprinter[i] - 0.15, sprinters_lengths[i] - 0.015, sprinter_names[i], fontsize = 8)

    x_values_marathoners = 1 + np.array((0.03, 0.06, 0.09, 0.12, 0.15, 0.18, 0.21))
    for i in range(len(marathoners_names)):
        ax.scatter(x_values_marathoners[i], marathoners_lengths[i], color=colormap_5[2], s=10)
        if i == 5:
            ax.text(x_values_marathoners[i] +0.02, marathoners_lengths[i] - 0.005, marathoners_names[i], fontsize=8)
        elif i == 2:
            ax.text(x_values_marathoners[i] - 0.10, marathoners_lengths[i] + 0.005, marathoners_names[i], fontsize=8)
        elif i < 2:
            ax.text(x_values_marathoners[i] - 0.05,  marathoners_lengths[i] + 0.005,marathoners_names[i], fontsize = 8)
        elif i != 5:
            ax.text(x_values_marathoners[i] - 0.08, marathoners_lengths[i] + 0.005, marathoners_names[i], fontsize=8)
        else:
            ax.text(x_values_marathoners[i], marathoners_lengths[i] + 0.005, marathoners_names[i], fontsize=8)
    ...
    ax.set_xticklabels([*bodies, bodies[0]], fontsize = 13)
    ...
    plt.savefig('figures_paper/SKEL_scaling.svg',format = 'svg')
    plt.savefig('figures_paper/SKEL_scaling.jpg',format = 'jpg', dpi = 600)
    ...
    for i in range(len(cases_models)):
        ...
        maximal_isometric_torques_overall = np.concatenate(
            (maximal_isometric_torques_overall, np.reshape(maximal_isometric_torques_overall[0], (1,))))
    ...
    ax.set_xticklabels([*labels_max_iso_torque, labels_max_iso_torque[0]],fontsize = 13)
    ...
    ax.text(0.46, 0.95, '1.0', transform=ax.transAxes)
    angle_annotation = np.pi/2
    ax.plot([angle_annotation, angle_annotation], [1, 1.3], color='k', linestyle='-', linewidth=1)
    ax.plot(label_loc_unit_circle, unit_circle, label='unit circle', color=(0, 0, 0), linewidth=1)

    plt.legend(labels_models[:3], loc='upper left', bbox_to_anchor=(1.5, 0.75))
    plt.title('joint torque capacity')
...
# generate legend
plt.legend(labels_models_MUSC,loc='upper left',bbox_to_anchor=(1,0.5), fontsize = 10)
plt.title('sprinting speed [m/s]')
...
for i in range(len(cases_marathon_MUSC)):
    y_value = np.around(optimaltrajectories[cases_marathon_MUSC[i]]['stats']['iterations']['obj'][-1], decimals=0).flatten()
    bar_i = ax.bar(x = x_values[i] , height = y_value, width = 0.45, color = colormap_MUSC[i], label = labels_models[i])
    ax.bar_label(bar_i) # add bar label
...
plt.legend(labels_models_MUSC,loc='upper left',bbox_to_anchor=(1,0.5), fontsize = 10)
plt.title('cost')
plt.savefig('figures_paper/MUSC_cost.svg',format = 'svg')
plt.savefig('figures_paper/MUSC_cost.jpg',format = 'jpg', dpi = 600)
...
plt.legend(labels_models_MUSC[:3],loc='upper left',bbox_to_anchor=(1.5,0.75))
plt.title('maximal isometric torque [-] \n normalized to generic')
plt.savefig('figures_paper/MUSC_max_iso_torque_simple.svg',format = 'svg')
plt.savefig('figures_paper/MUSC_max_iso_torque_simple.jpg',format = 'jpg', dpi =1200)
...
for i in range(len(cases_models_MUSC)):
    # maximal_isometric_torques_generic = optimaltrajectories[cases_models_MUSC[0]]['maximal_isometric_torques']
    maximal_isometric_torques = optimaltrajectories[cases_models_MUSC[i]]['maximal_isometric_torques']
    Qs_max_iso_torque = optimaltrajectories[cases_models_MUSC[i]]['Qs_max_iso_torque']
    ax.plot(Qs_max_iso_torque[13,:,0], maximal_isometric_torques[:,0], label=labels_models[i], color = colormap_MUSC[i])
...
for i in range(len(cases_models_MUSC)):
    ...
    maximal_isometric_torques = optimaltrajectories[cases_models_MUSC[i]]['maximal_isometric_torques']
    Qs_max_iso_torque = optimaltrajectories[cases_models_MUSC[i]]['Qs_max_iso_torque']
    ax.plot(Qs_max_iso_torque[13,:,0], maximal_isometric_torques[:,1], label=labels_models[i], color = colormap_MUSC[i])
...
for i in range(len(cases_models_MUSC)):
    ...
    maximal_isometric_torques = optimaltrajectories[cases_models_MUSC[i]]['maximal_isometric_torques']
    Qs_max_iso_torque = optimaltrajectories[cases_models_MUSC[i]]['Qs_max_iso_torque']
    ax.plot(Qs_max_iso_torque[16,:,2], maximal_isometric_torques[:,2], label=labels_models[i], color = colormap_MUSC[i])
...
for i in range(len(cases_models_MUSC)):
    ...
    maximal_isometric_torques = optimaltrajectories[cases_models_MUSC[i]]['maximal_isometric_torques']
    Qs_max_iso_torque = optimaltrajectories[cases_models_MUSC[i]]['Qs_max_iso_torque']
    ax.plot(Qs_max_iso_torque[16,:,2], maximal_isometric_torques[:,3], label=labels_models[i], color = colormap_MUSC[i])
...
for i in range(len(cases_models_MUSC)):
    ...
    maximal_isometric_torques = optimaltrajectories[cases_models_MUSC[i]]['maximal_isometric_torques']
    Qs_max_iso_torque = optimaltrajectories[cases_models_MUSC[i]]['Qs_max_iso_torque']
    ax.plot(Qs_max_iso_torque[17,:,4], maximal_isometric_torques[:,4], label=labels_models[i], color = colormap_MUSC[i])
...
for i in range(len(cases_models_MUSC)):
    ...
    maximal_isometric_torques = optimaltrajectories[cases_models_MUSC[i]]['maximal_isometric_torques']
    Qs_max_iso_torque = optimaltrajectories[cases_models_MUSC[i]]['Qs_max_iso_torque']
    ax.plot(Qs_max_iso_torque[17,:,4], maximal_isometric_torques[:, 5], label=labels_models[i], color=colormap_MUSC[i])
...
# Generalized forces - pelvis
pelvis_joints = ['pelvis_tilt','pelvis_list','pelvis_rotation','pelvis_tx','pelvis_ty','pelvis_tz']
for index,pelvis_joint in enumerate(pelvis_joints):

    ax = fig.add_subplot(2,3,index+1)
    for i in range(len(cases_sprinting_MUSC)):
        joint_names = optimaltrajectories[cases_sprinting_MUSC[i]]['joint_names']
        joint_torques = optimaltrajectories[cases_sprinting_MUSC[i]]['generalized_forces']
        joint_torques_gait_cycle, joint_names_reduced = utilities.get_full_gait_cycle_joint_variables(joint_names, joint_torques)
        index_coordinate = joint_names_reduced.index(pelvis_joint)
        time = optimaltrajectories[cases_sprinting_MUSC[i]]['time_col_opt']
        time_gait_cycle = np.concatenate((np.zeros((1,)),time[:-1],time+time[-1]))
        ax.plot(time_gait_cycle, joint_torques_gait_cycle[index_coordinate,:], label=labels_models[i], color = colormap_MUSC[i])
        ax.set_title(pelvis_joint)


fig = plt.figure(figsize=(10,10))
# Generalized forces - joint torque hip - knee - ankle
selected_joints = ['hip_flexion','hip_adduction','hip_rotation','knee_angle','ankle_angle','subtalar_angle']
for index,selected_joint in enumerate(selected_joints):
    ...
    for i in range(len(cases_marathon_MUSC)):
        ...
        joint_torques = optimaltrajectories[cases_sprinting_MUSC[i]]['generalized_forces']
        joint_torques_gait_cycle, joint_names_reduced = utilities.get_full_gait_cycle_joint_variables(joint_names, joint_torques)
        index_coordinate = joint_names_reduced.index(selected_joint)
        time = optimaltrajectories[cases_sprinting_MUSC[i]]['time_col_opt']
        time_gait_cycle = np.concatenate((np.zeros((1,)), time[:-1], time + time[-1]))
        ax.plot(time_gait_cycle,joint_torques_gait_cycle[index_coordinate,:], label=labels_models[i], color = colormap_MUSC[i])
        ax.set_title(selected_joint)


fig = plt.figure(figsize=(7,2.5))

# Generalized forces - joint torque hip - knee - ankle
selected_joints = ['hip_flexion','knee_angle','ankle_angle']
selected_joint_torque_names = ['hip torque [Nm]','knee torque [Nm]','ankle torque [Nm]']
for index,selected_joint in enumerate(selected_joints):

    if selected_joint == 'knee_angle':
        flex_ext_multiplier = -1
    else:
        flex_ext_multiplier = 1
    ...
    for i in range(len(cases_marathon_MUSC)):
        ...
        joint_torques = optimaltrajectories[cases_sprinting_MUSC[i]]['generalized_forces']
        joint_torques_gait_cycle, joint_names_reduced = utilities.get_full_gait_cycle_joint_variables(joint_names, joint_torques)
        joint_torques_gait_cycle = np.concatenate((joint_torques_gait_cycle[:,12:], joint_torques_gait_cycle[:,:12]),1)
        index_coordinate = joint_names_reduced.index(selected_joint)
        time = optimaltrajectories[cases_sprinting_MUSC[i]]['time_col_opt']
        time_gait_cycle = np.concatenate((np.zeros((1,)), time[:-1], time + time[-1]))
        ax.plot(time_gait_cycle,flex_ext_multiplier*joint_torques_gait_cycle[index_coordinate,:], label=labels_models[i], color = colormap_MUSC[i])
    ax.set_title(selected_joint_torque_names[index])

    if selected_joint == 'hip_flexion':
        # no xtick labels
        ax.set_xticks([0,0.1,0.5])
        ax.set_xticklabels(['0','0.1','0.5'])
        ax.set_xlim([0, 0.5])
        # ytick labels
        ax.set_yticks([-300,0,250])
        ax.set_yticklabels(['-300','0','250'])
        ax.set_ylim([-300,250])
    ...
    if selected_joint == 'knee_angle':
        ...
        ax.set_xlim([0, 0.5])
        # ytick labels
        ax.set_yticks([-100, 0, 120])
        ax.set_yticklabels(['-100','0','120'])
        ax.set_ylim([-100,120])

    if selected_joint == 'ankle_angle':
        # no xtick labels
        ax.set_xticks([0,0.1,0.5])
        ax.set_xticklabels(['0','0.1','0.5'])
        ax.set_xlim([0, 0.5])
        # ytick labels
        ax.set_yticks([-320, 0, 50])
        ax.set_yticklabels(['-320', '0', '50'])
        ax.set_ylim([-320, 50])
...
plt.savefig('figures_paper/MUSC_joint_torques_simple.svg',format = 'svg')
plt.savefig('figures_paper/MUSC_joint_torques_simple.jpg',format = 'jpg', dpi =1200)

fig = plt.figure(figsize=(10,10))
# Generalized forces - joint torque hip - knee - ankle
selected_joints = ['lumbar_extension','lumbar_rotation','lumbar_bending','arm_flex','arm_add','elbow_flex']
for index,selected_joint in enumerate(selected_joints):
    ...
    for i in range(len(cases_marathon_MUSC)):
        ...
        index_coordinate = joint_names_reduced.index(selected_joint)
        time = optimaltrajectories[cases_sprinting_MUSC[i]]['time_col_opt']
        ax.plot(joint_torques_gait_cycle[index_coordinate,:], label=labels_models_MUSC[i], color = colormap_MUSC[i])
        ax.set_title(selected_joint)
...
##### muscle names simple
hip_abduction_simple = ['glut_max1_r', 'glut_med1_r', 'glut_med2_r', 'glut_med3_r', 'glut_min1_r',
                  'glut_min2_r', 'glut_min3_r']

hip_adduction_simple = ['add_brev_r', 'add_long_r', 'add_mag1_r', 'add_mag2_r', 'add_mag3_r',
                  'bifemlh_r', 'grac_r', 'pect_r', 'semimem_r', 'semiten_r']

hip_extension_simple = ['add_long_r', 'add_mag1_r',
                  'bifemlh_r', 'glut_max1_r', 'glut_med3_r',
                 'semimem_r','semiten_r']
hip_extension_simple_names = ['add long', 'add mag ',
                  'bifemlh', 'glut max', 'post\nglut med',
                 'semimem','semiten']

hip_flexion_simple = ['add_long_r', 'glut_med1_r', 'iliacus_r', 'pect_r', 'psoas_r', 'rect_fem_r', 'tfl_r']
hip_flexion_simple_names = ['add long', 'ant\nglut med.', 'iliacus', 'pect', 'psoas', 'rect fem', 'tfl']
...
knee_flexion_simple = ['bifemlh_r', 'bifemsh_r', 'lat_gas_r', 'med_gas_r','semimem_r','semiten_r']
knee_flexion_simple_names = ['bifemlh', 'bifemsh', 'lat gas', 'med gas','semimem','semiten']

knee_extension_simple = ['rect_fem_r', 'vas_int_r', 'vas_lat_r', 'vas_med_r']
knee_extension_simple_names = ['rect fem', 'vas int', 'vas lat', 'vas med']

ankle_plantarflexion_simple = ['flex_dig_r', 'flex_hal_r', 'lat_gas_r', 'med_gas_r','soleus_r','tib_post_r']
ankle_plantarflexion_simple_names = ['flex dig', 'flex hal', 'lat gas', 'med gas.','soleus','tib post']

ankle_dorsiflexion_simple = ['ext_dig_r', 'ext_hal_r', 'per_tert_r', 'tib_ant_r']
ankle_dorsiflexion_simple_names = ['ext dig', 'ext hal', 'per tert', 'tib ant']


ankle_eversion_simple = ['ext_dig_r', 'per_brev_r','per_long_r', 'per_tert_r']
ankle_inversion_simple = ['ext_hal_r', 'flex_dig_r', 'flex_hal_r', 'tib_ant_r', 'tib_post_r']


muscle_actuations = [hip_flexion, knee_flexion, ankle_dorsiflexion, hip_extension, knee_extension, ankle_plantarflexion]
muscle_actuations_simple = [hip_flexion_simple, knee_flexion_simple, ankle_dorsiflexion_simple, hip_extension_simple, knee_extension_simple, ankle_plantarflexion_simple]
muscle_actuations_simple_names = [hip_flexion_simple_names, knee_flexion_simple_names, ankle_dorsiflexion_simple_names, hip_extension_simple_names, knee_extension_simple_names, ankle_plantarflexion_simple_names]

muscle_actuations_string = ['hip flexion', 'knee flexion', 'ankle dorsiflexion', 'hip extension', 'knee extension', 'ankle plantarflexion']

fig = plt.figure(figsize=(10,5))

for m, muscle_actuation_simple in enumerate(muscle_actuations_simple):

    ax = fig.add_subplot(2,3,m+1, projection='polar')


    for i in range(len(cases_models_MUSC)):
        muscle_names = optimaltrajectories[cases_models_MUSC[i]]['muscle_names']
        muscle_PCSA_scaling = optimaltrajectories[cases_models_MUSC[i]]['muscle_cross_section_multiplier_opt']

        indices = []
        for j in range(len(muscle_actuation_simple)):
            indices.append(muscle_names.index(muscle_actuation_simple[j]))
        ...
        ax.set_xticklabels([*muscle_actuations_simple_names[m], muscle_actuations_simple_names[m][0]])
        ...
        ax.plot(label_loc, muscle_PCSA_scaling[indices], label=labels_models_MUSC[i], color=colormap_MUSC[i])
    ...
    ax.plot([angle_annotation, angle_annotation], [1, 1.3], color='k', linestyle='-', linewidth=1)
    plt.title(muscle_actuations_string[m])

fig.tight_layout()

plt.savefig('figures_paper/MUSC_muscle_force.svg',format = 'svg')
plt.savefig('figures_paper/MUSC_muscle_force.jpg',format = 'jpg', dpi =1200)
