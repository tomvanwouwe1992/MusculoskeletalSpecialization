...
class Konverter:
  def __init__(self, input_model=None, output_file=None, indent=2, silent=True, no_watermark=False, tf_verbose=False):
    if not tf_verbose:
  ...
  def build_konverted_model(self):
    ...
    model_builder = {'imports': ['import numpy as np', 'import casadi as ca'],
                     'functions': [],
                     'load_weights': [],
                     'model': ['def predict(x):',
                               'x = ca.vertcat(x)']}  # convert input to float32
