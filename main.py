...
for case in cases:
    ...
    path_trajectories = os.path.join(path_main, 'Results')
    ...
    if 'bounds_cross_sectional_area_scaling' in settings[case]:
        bounds_cross_sectional_area_scaling = settings[case]['bounds_cross_sectional_area_scaling']


    if isinstance(bounds_cross_sectional_area_scaling, str):
        case_to_load = bounds_cross_sectional_area_scaling[-1]
        optimaltrajectories = np.load(os.path.join(path_trajectories, 'optimaltrajectories.npy'),
                allow_pickle=True)
        optimaltrajectories = optimaltrajectories.item()
        cross_sectional_area_scaling = optimaltrajectories[case_to_load]['muscle_cross_section_multiplier_opt']
        bounds_cross_sectional_area_scaling = [cross_sectional_area_scaling, cross_sectional_area_scaling]
    ...
    if 'NeMu_folder' in settings[case]:
        NeMu_folder = os.path.dirname(
            os.path.abspath('main.py')) + '/Models/' + settings[case]['NeMu_folder']
    ...
    ### Muscle volume (original and scaled)
    f_muscle_volume = casadiFunctions.get_muscle_volume_and_scaled_volume(maximalIsometricForce,
    ...
    if solveProblem:
        ...
        if strength_training == True:
            ...
            opti.subject_to(
                opti.bounded(bounds_cross_sectional_area_scaling[0], muscle_cross_section_multiplier_opti[:, 0], bounds_cross_sectional_area_scaling[1]))
            ...
            [muscleVolume_original, muscleVolume_scaled, optimalFiberLength_scaled, maximalIsometricForce_scaled] = f_muscle_volume(muscle_length_scaling_vector_opti[:, 0], model_mass_scaling_opti[:, 0],
                                        muscle_cross_section_multiplier_opti[:, 0])

            scaled_total_muscle_volume = ca.sum1(muscleVolume_scaled)
            original_total_muscle_volume = ca.sum1(muscleVolume_original)
            ratio_muscle_volume = scaled_total_muscle_volume / original_total_muscle_volume
            opti.subject_to(opti.bounded(0.95, ratio_muscle_volume, 1.05))

        else:
            muscle_cross_section_multiplier_opti = opti.variable(number_of_muscles, polynomial_order * number_of_mesh_intervals)
            for j in range(0, polynomial_order * number_of_mesh_intervals):
                opti.subject_to(
                    opti.bounded(bounds_cross_sectional_area_scaling[0], muscle_cross_section_multiplier_opti[:, j], bounds_cross_sectional_area_scaling[1]))
            opti.set_initial(muscle_cross_section_multiplier_opti, 1)
