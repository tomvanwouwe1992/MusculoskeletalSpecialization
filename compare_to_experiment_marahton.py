import numpy, os, matplotlib, opensim, scipy
...
scaling_vector = optimaltrajectories['13']['skeleton_scaling']
Q_at_collocation = optimaltrajectories['13']['Q_at_collocation']
time_col_opt = optimaltrajectories['13']['time_col_opt']
joint_names = optimaltrajectories['13']['joint_names']
...
left_GRF_full_part_1 = optimaltrajectories['13']['external_function_ouput'][34:37,:]
left_GRF_full_part_2 = optimaltrajectories['13']['external_function_ouput'][31:34,:]
left_GRF_full_original = numpy.concatenate((left_GRF_full_part_1.T, left_GRF_full_part_2.T))



left_GRF_part_1 = optimaltrajectories['13']['external_function_ouput'][35,:]
left_GRF_part_2 = optimaltrajectories['13']['external_function_ouput'][32,:]
...
left_foot_on_index = left_GRF_non_zeros[0]-1
...
subject_weights = [72.84, 76.48, 64.00, 80.47, 82.38 ,69.26, 69.35, 68.45, 65.14, 67.18]
all_GRF_data = []
all_GRF_data_full = []
for k in range(10):
    root_path = os.path.dirname(os.path.abspath('compare_to_experiment_marahton.py'))
    if subject_names[k] == 'subject08' or subject_names[k] == 'subject17' or subject_names[k] == 'subject19'or subject_names[k] == 'subject20':
        motion_file = os.path.join(root_path, 'Hamner_running_data',subject_names[k],'ik','Results_191','Run_30001.mot')
        GRF_file = os.path.join(root_path, 'Hamner_running_data', subject_names[k] ,'ExportedData','Run_300 01_newCOP3.mot')
    else:
        motion_file = os.path.join(root_path, 'Hamner_running_data',subject_names[k],'ik','Results_191','Run_30002.mot')
        GRF_file = os.path.join(root_path, 'Hamner_running_data', subject_names[k] ,'ExportedData','Run_300 02_newCOP3.mot')
    ...
    GRF_data_full = numpy.zeros((num_time_steps, 4))
     # left vertical GRF
    for i in range(0,num_time_steps):
        GRF_data[i ,1] = GRF.getStateVector(i).getData().get(6)
        GRF_data[i, 0] = GRF.getStateVector(i).getTime()
        GRF_data_full[i, 0] = GRF.getStateVector(i).getTime()
        GRF_data_full[i, 1] = GRF.getStateVector(i).getData().get(6)
        GRF_data_full[i, 2] = GRF.getStateVector(i).getData().get(7)
        GRF_data_full[i, 3] = GRF.getStateVector(i).getData().get(8)
...
for i in range(34):
    if column_names[i] in joints_of_interest:
        ...
        for k in range(len(all_motion_data)):
            ...
            ax.plot(100*time_resampled/time_resampled[-1], motion_data_resampled,'--', color='black', alpha=0.8,linewidth=0.3)
        ...
        if column_names[i] in joints_of_interest:
            ...
            ax.plot(100*time_col_opt_resampled/time_col_opt_resampled[-1], Q_of_interest[:,index],color=colormap_5[0],linewidth=2.0)
...
plt.savefig('figures_paper/experimental_validation_marathon_stance_first_vJuly.svg', format='svg')
plt.savefig('figures_paper/experimental_validation_marathon_stance_first_vJuly.jpg', format='jpg', dpi=600)
...
for i in range(3):
    ...
    for k in range(len(all_motion_data)):
        ...
        ax.plot(100*time_resampled/time_resampled[-1], GRF_data_resampled/subject_weights[k]/9.81,'--', color='black', alpha=0.8,linewidth=0.3)
        title = titles_GRF[i]
        # if title[-1] == 'l':
        #     title = title[:-2]
        # title = title.replace('_',' ')
...
plt.savefig('figures_paper/experimental_validation_marathon_stance_first_GRF.svg', format='svg')
plt.savefig('figures_paper/experimental_validation_marathon_stance_first_GRF.jpg', format='jpg', dpi=600)
